module ownership::ownership {
    ...
    const UID_DOES_NOT_BELONG_TO_OBJECT: u64 = 3;
    const EMODULE_ALREADY_EXISTS: u64 = 4;
    const EOBJECT_NOT_INITIALIZED: u64 = 5;
    ...
    public fun initialize<T: key>(uid: &mut UID, obj: &T, auth: &TxAuthority) {
        ...
        initialize(uid, obj, module_authority, auth);
    }

    public fun initialize_<T: key>(uid: &mut UID, obj: &T, module_authority: address, auth: &TxAuthority) {
        assert!(object::uid_to_inner(uid) == object::id_address(obj), EUID_DOES_NOT_BELONG_TO_OBJECT);
    ...
    public fun bind_transfer(uid: &mut UID, addr: address, auth: &TxAuthority) {
        ...
        assert!(is_authorized_by_module(uid, auth), ENO_CREATOR_AUTHORITY);
        assert!(is_authorized_by_owner(uid, auth), ENO_OWNER_AUTHORITY);
    ...
    public fun unbind_transfer(uid: &mut UID, auth: &TxAuthority) {
        assert!(is_authorized_by_transfer(uid, auth), ENO_CREATOR_AUTHORITY);
        assert!(is_authorized_by_owner(uid, auth), ENO_OWNER_AUTHORITY);
    ...
    public fun bind_owner(uid: &mut UID, owner: address, auth: &TxAuthority) {
        ...
        dynamic_field::add(id, Owner { }, owner);        
    }

    // Requires permission from transfer
    public fun unbind_owner(uid: &mut UID, owner: address, auth: &TxAuthority) {
        assert!(is_authorized_by_transfer(uid, auth), ENO_CREATOR_AUTHORITY);
    ...
    public fun transfer(id: &mut UID, new_owner: address, auth: &TxAuthority) {
        ...
        *dynamic_field::borrow_mut<Owner, address>(id, Owner { }) = new_owner;
    }
    ...
    public fun get_module(uid: &UID): Option<address> {
        if (dynamic_field::exists_(uid, Module { })) {
            let addr = object::id_from_address(dynamic_field::borrow<Module, address>(uid, Module { }))
            option::some(addr)
        } else {
            option::none()
        }
