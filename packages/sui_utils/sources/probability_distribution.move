...
module sui_utils::probability_distribution {
    ...
    const MAX_LAMBDA: u64 = 18446744073709551615 / PRECISION;
    ...
    public fun sample_from_distribution(curve: &ProbabilityDistribution): u64 {
    ...
    public fun normal(mean: u64, std_dev: u64, min: u64, max: u64): u64 {
        assert!(max > min, EINVALID_ARGUMENTS);

        let (sum, i) = (0, 0);
        while (i < SAMPLE_SIZE) {
            sum = sum + rand::rng(min, max);
            i = i + 1;
        };
        ...
        if (avg > mean) {
            ...
            if (x > (max as u128)) { return max } else { return max((x as u64), main) }
        } else {
    ...
    public fun exponential(lambda: u64, min: u64, max: u64, weighted_high: bool): u64 {
        assert!(max > min, EINVALID_ARGUMENTS);
        assert!(lambda > 0 && lambda <= MAX_LAMBDA, EINVALID_ARGUMENTS);

        let uniform_value = rand::rng(1, PRECISION);
