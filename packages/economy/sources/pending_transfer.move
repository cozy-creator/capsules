...
module economy::pending_transfer {
    ...
    const EPENDING_TRANSFER_ALREADY_CREATED: u64 = 2;
    ...
    public fun add_object(
        ...
        assert!(!pending.locked, EPENDING_TRANSFER_ALREADY_CREATED);
        assert!(ownership::can_act_as_transfer_auth<TRANSFER>(uid, auth), ENO_TRANSFER_AUTH);
    ...
    public fun begin_transfer(pending: PendingTransfer, uid: &mut UID, clock: &Clock): HotPotato {
        ...
        let potato = HotPotato { contents };
        continue_transfer(&mut potato, uid);
        potato
    }

    // We cannot delete `PendingTransfer` yet, because it's a shared object
    public fun begin_transfer(pending: &mut PendingTransfer, uid: &mut UID, clock: &Clock): HotPotato {
        ...
        let potato = HotPotato { contents: vector[] };
        while (vector::length(&pending.contents) > 0) {
            vector_push(&mut potato, vector::pop(&mut pending.contents));
        };
    ...
    public fun continue_transfer(potato: &mut HotPotato, uid: &mut UID) {
        let id = object::uid_to_id(uid);
        let (_, recipient) = vec_map::remove(&mut potato.contents, &id);
        ownership::transfer_internal(uid, recipient);
    }

    public fun conclude_transfer(potato: HotPotato) {
        let HotPotato { contents } = potato;
