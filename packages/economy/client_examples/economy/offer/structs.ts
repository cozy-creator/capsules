...
bcs.registerStructType(
  "0x77e53a48851512fe5707346cf90f85ae0c46b6b527b0fa04935940b0e2f38b42::offer::Witness",
  {
    dummy_field: `bool`,
  },
);
...
bcs.registerStructType(
  "0x77e53a48851512fe5707346cf90f85ae0c46b6b527b0fa04935940b0e2f38b42::offer::Offer<T>",
  {
    id: `0x2::object::UID`,
    send_to: `address`,
    claim: `0x1::option::Option<0x77e53a48851512fe5707346cf90f85ae0c46b6b527b0fa04935940b0e2f38b42::claim::Claim<T>>`,
...
export function isOffer(type: Type): boolean {
  return type.startsWith(
    "0x77e53a48851512fe5707346cf90f85ae0c46b6b527b0fa04935940b0e2f38b42::offer::Offer<",
  );
}
...
export class Offer {
  ...
  constructor(typeArg: Type, fields: OfferFields) {
  ...
  static fromFields(typeArg: Type, fields: Record<string, any>): Offer {
    return new Offer(typeArg, {
      id: UID.fromFields(fields.id).id,
      sendTo: `0x${fields.send_to}`,
      claim:
        Option.fromFields<Claim>(
          `0x77e53a48851512fe5707346cf90f85ae0c46b6b527b0fa04935940b0e2f38b42::claim::Claim<${typeArg}>`,
          fields.claim,
        ).vec[0] || null,
      forId:
  ...
  static fromFieldsWithTypes(item: FieldsWithTypes): Offer {
    if (!isOffer(item.type)) {
      throw new Error("not a Offer type");
    }
    const { typeArgs } = parseTypeName(item.type);

    return new Offer(typeArgs[0], {
      ...
      claim:
        item.fields.claim !== null
          ? Option.fromFieldsWithTypes<Claim>({
              type:
                "0x1::option::Option<" +
                `0x77e53a48851512fe5707346cf90f85ae0c46b6b527b0fa04935940b0e2f38b42::claim::Claim<${typeArgs[0]}>` +
                ">",
  ...
  static fromBcs(
    ...
    return Offer.fromFields(
      typeArg,
      bcs.de([Offer.$typeName, typeArg], data, encoding),
    );
  }
  ...
  static fromSuiParsedData(content: SuiParsedData) {
    ...
    if (!isOffer(content.type)) {
      throw new Error(`object at ${content.fields.id} is not a Offer object`);
    }
    return Offer.fromFieldsWithTypes(content);
  }

  static async fetch(provider: JsonRpcProvider, id: ObjectId): Promise<Offer> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching Offer object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.content?.dataType !== "moveObject" ||
      !isOffer(res.data.content.type)
    ) {
      throw new Error(`object at id ${id} is not a Offer object`);
    }
    return Offer.fromFieldsWithTypes(res.data.content);
  }
}
