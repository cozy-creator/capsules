...
export class LinkedTable<K> {
  ...
  static fromSuiParsedData(content: SuiParsedData) {
    ...
    if (!isLinkedTable(content.type)) {
      throw new Error(
        `object at ${content.fields.id} is not a LinkedTable object`,
      );
    }
  ...
  static async fetch<K>(
    ...
    if (
      ...
      !isLinkedTable(res.data.content.type)
    ) {
...
export class Node<K, V> {
    ...
    return new Node([typeArgs[0], typeArgs[1]], {
      ...
      next:
        item.fields.next !== null
          ...
          : null,
      value: structClassLoaderSource.fromFieldsWithTypes(
        typeArgs[1],
        item.fields.value,
      ),
    });
  ...
  static fromBcs<K, V>(
    ...
    return Node.fromFields(
      typeArgs,
      bcs.de([Node.$typeName, ...typeArgs], data, encoding),
    );
  }
}
