...
export class Publisher {
  ...
  static fromSuiParsedData(content: SuiParsedData) {
    ...
    if (!isPublisher(content.type)) {
      throw new Error(
        `object at ${content.fields.id} is not a Publisher object`,
      );
    }
  ...
  static async fetch(
    ...
    if (
      ...
      !isPublisher(res.data.content.type)
    ) {
...
export class UpgradeCap {
  ...
  static fromBcs(data: Uint8Array | string, encoding?: Encoding): UpgradeCap {
    return UpgradeCap.fromFields(
      bcs.de([UpgradeCap.$typeName], data, encoding),
    );
  }
  ...
  static fromSuiParsedData(content: SuiParsedData) {
    ...
    if (!isUpgradeCap(content.type)) {
      throw new Error(
        `object at ${content.fields.id} is not a UpgradeCap object`,
      );
    }
  ...
  static async fetch(
    ...
    if (
      res.data?.content?.dataType !== "moveObject" ||
      !isUpgradeCap(res.data.content.type)
    ) {
...
export class UpgradeReceipt {
  ...
  static fromBcs(
    ...
    return UpgradeReceipt.fromFields(
      bcs.de([UpgradeReceipt.$typeName], data, encoding),
    );
  }
}
...
export class UpgradeTicket {
  ...
  static fromBcs(
    ...
    return UpgradeTicket.fromFields(
      bcs.de([UpgradeTicket.$typeName], data, encoding),
    );
  }
}
