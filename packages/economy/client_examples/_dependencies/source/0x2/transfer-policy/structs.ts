...
export class RuleKey {
  ...
  static fromBcs(
    ...
    return RuleKey.fromFields(
      typeArg,
      bcs.de([RuleKey.$typeName, typeArg], data, encoding),
    );
  }
}
...
export class TransferPolicy {
  ...
  static fromFields(
    ...
    return new TransferPolicy(typeArg, {
      ...
      rules: VecSet.fromFields<TypeName>(
  ...
  static fromSuiParsedData(content: SuiParsedData) {
    ...
    if (!isTransferPolicy(content.type)) {
      throw new Error(
        `object at ${content.fields.id} is not a TransferPolicy object`,
      );
    }
  ...
  static async fetch(
    ...
    if (
      ...
      !isTransferPolicy(res.data.content.type)
    ) {
...
export class TransferPolicyCap {
  ...
  static fromSuiParsedData(content: SuiParsedData) {
    ...
    if (!isTransferPolicyCap(content.type)) {
      throw new Error(
        `object at ${content.fields.id} is not a TransferPolicyCap object`,
      );
    }
...
export class TransferRequest {
  ...
  static fromFields(
    ...
    return new TransferRequest(typeArg, {
      ...
      receipts: VecSet.fromFields<TypeName>(
        `0x1::type_name::TypeName`,
        fields.receipts,
      ),
    });
