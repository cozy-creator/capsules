...
export interface AddPackageWitness_Args {
  witness: GenericArg;
  auth: ObjectArg;
}

export function addPackageWitness_(
  txb: TransactionBlock,
  typeArg: Type,
  args: AddPackageWitness_Args,
) {
  return txb.moveCall({
    target: `${PUBLISHED_AT}::tx_authority::add_package_witness_`,
    typeArguments: [typeArg],
    arguments: [generic(txb, `${typeArg}`, args.witness), obj(txb, args.auth)],
  });
}
...
export function beginWithPackageWitness_(
  txb: TransactionBlock,
  typeArg: Type,
  witness: GenericArg,
) {
  return txb.moveCall({
    target: `${PUBLISHED_AT}::tx_authority::begin_with_package_witness_`,
    typeArguments: [typeArg],
    arguments: [generic(txb, `${typeArg}`, witness)],
  });
}
