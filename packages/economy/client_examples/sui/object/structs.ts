...
export class DynamicFields<K> {
  ...
  static fromSuiParsedData(content: SuiParsedData) {
    ...
    if (!isDynamicFields(content.type)) {
      throw new Error(
        `object at ${content.fields.id} is not a DynamicFields object`,
      );
    }
    return DynamicFields.fromFieldsWithTypes(content);
  }

  static async fetch<K>(
    provider: JsonRpcProvider,
    id: ObjectId,
  ): Promise<DynamicFields<K>> {
    ...
    if (res.error) {
      throw new Error(
        `error fetching DynamicFields object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.content?.dataType !== "moveObject" ||
      !isDynamicFields(res.data.content.type)
    ) {
...
export class Ownership {
  ...
  static fromFieldsWithTypes(item: FieldsWithTypes): Ownership {
    ...
    return new Ownership({
      ...
      status: BigInt(item.fields.status),
    });
  }
  ...
  static fromSuiParsedData(content: SuiParsedData) {
    ...
    if (!isOwnership(content.type)) {
      throw new Error(
        `object at ${content.fields.id} is not a Ownership object`,
      );
    }
  ...
  static async fetch(
    ...
    if (res.error) {
      throw new Error(
        `error fetching Ownership object at id ${id}: ${res.error.code}`,
      );
    }
    if (
      res.data?.content?.dataType !== "moveObject" ||
      !isOwnership(res.data.content.type)
    ) {
