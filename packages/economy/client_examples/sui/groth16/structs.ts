...
export class PreparedVerifyingKey {
  ...
  static fromFields(fields: Record<string, any>): PreparedVerifyingKey {
    return new PreparedVerifyingKey({
      ...
      alphaG1BetaG2Bytes: fields.alpha_g1_beta_g2_bytes.map(
        (item: any) => item,
      ),
  ...
  static fromFieldsWithTypes(item: FieldsWithTypes): PreparedVerifyingKey {
    ...
    return new PreparedVerifyingKey({
      ...
      alphaG1BetaG2Bytes: item.fields.alpha_g1_beta_g2_bytes.map(
        (item: any) => item,
      ),
      gammaG2NegPcBytes: item.fields.gamma_g2_neg_pc_bytes.map(
        (item: any) => item,
      ),
      deltaG2NegPcBytes: item.fields.delta_g2_neg_pc_bytes.map(
        (item: any) => item,
      ),
    });
  }
  ...
  static fromBcs(
    ...
    return PreparedVerifyingKey.fromFields(
      bcs.de([PreparedVerifyingKey.$typeName], data, encoding),
    );
  }
}
...
export class ProofPoints {
  ...
  static fromBcs(data: Uint8Array | string, encoding?: Encoding): ProofPoints {
    return ProofPoints.fromFields(
      bcs.de([ProofPoints.$typeName], data, encoding),
    );
  }
}
...
export class PublicProofInputs {
  ...
  static fromBcs(
    ...
    return PublicProofInputs.fromFields(
      bcs.de([PublicProofInputs.$typeName], data, encoding),
    );
  }
}
