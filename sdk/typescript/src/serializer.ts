...
bcs.registerType(
  ...
  (reader) => {
    ...
    reader.readVec((reader) => {
      let key = new TextDecoder("utf8").decode(
        reader.readBytes(reader.readULEB())
      );
      let value = new TextDecoder("utf8").decode(
        reader.readBytes(reader.readULEB())
      );
      data[key] = value;
    });
...
const schemaToStringArray = (
  schema: Record<string, SupportedJSTypes>
): string[][] => {
