...
export class PublishReceipt {
    ...
    static fromBcs(data: Uint8Array | string, encoding?: Encoding): PublishReceipt {
        return PublishReceipt.fromFields(bcs.de([PublishReceipt.$typeName], data, encoding))
    }
    ...
    static fromSuiParsedData(content: SuiParsedData) {
        ...
        if (!isPublishReceipt(content.type)) {
            throw new Error(`object at ${content.fields.id} is not a PublishReceipt object`)
        }
