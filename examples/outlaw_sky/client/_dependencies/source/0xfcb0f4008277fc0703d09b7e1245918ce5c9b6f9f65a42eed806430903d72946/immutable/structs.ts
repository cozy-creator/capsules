...
bcs.registerStructType(
    "0xfcb0f4008277fc0703d09b7e1245918ce5c9b6f9f65a42eed806430903d72946::immutable::Immutable<T>",
    {
        id: `0x2::object::UID`,
        allow_read: `bool`,
        contents: `T`,
...
export function isImmutable(type: Type): boolean {
    return type.startsWith(
        "0xfcb0f4008277fc0703d09b7e1245918ce5c9b6f9f65a42eed806430903d72946::immutable::Immutable<"
    )
}
...
export class Immutable<T> {
    ...
    static fromFields<T>(typeArg: Type, fields: Record<string, any>): Immutable<T> {
        initLoaderIfNeeded()
    ...
    static fromBcs<T>(typeArg: Type, data: Uint8Array | string, encoding?: Encoding): Immutable<T> {
        return Immutable.fromFields(typeArg, bcs.de([Immutable.$typeName, typeArg], data, encoding))
    }
    ...
    static fromSuiParsedData(content: SuiParsedData) {
        ...
        if (!isImmutable(content.type)) {
            throw new Error(`object at ${content.fields.id} is not a Immutable object`)
        }
    ...
    static async fetch<T>(provider: JsonRpcProvider, id: ObjectId): Promise<Immutable<T>> {
        ...
        if (res.data?.content?.dataType !== "moveObject" || !isImmutable(res.data.content.type)) {
