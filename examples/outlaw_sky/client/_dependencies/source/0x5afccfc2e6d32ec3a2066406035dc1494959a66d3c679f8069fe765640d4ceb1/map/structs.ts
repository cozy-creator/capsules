...
bcs.registerStructType(
    "0x5afccfc2e6d32ec3a2066406035dc1494959a66d3c679f8069fe765640d4ceb1::map::Iter<Key>",
    {
        for: `0x2::object::ID`,
        keys: `vector<Key>`,
    }
...
export function isIter(type: Type): boolean {
    return type.startsWith(
        "0x5afccfc2e6d32ec3a2066406035dc1494959a66d3c679f8069fe765640d4ceb1::map::Iter<"
    )
}
...
export class Iter<Key> {
    ...
    static fromFields<Key>(typeArg: Type, fields: Record<string, any>): Iter<Key> {
        initLoaderIfNeeded()

        return new Iter(typeArg, {
            for: ID.fromFields(fields.for).bytes,
            keys: fields.keys.map((item: any) => structClassLoaderSource.fromFields(typeArg, item)),
        })
    }
    ...
    static fromBcs<Key>(typeArg: Type, data: Uint8Array | string, encoding?: Encoding): Iter<Key> {
        return Iter.fromFields(typeArg, bcs.de([Iter.$typeName, typeArg], data, encoding))
    }
}
...
bcs.registerStructType(
    "0x5afccfc2e6d32ec3a2066406035dc1494959a66d3c679f8069fe765640d4ceb1::map::Map<Key, Value>",
    {
        id: `0x2::object::UID`,
        index: `vector<Key>`,
    }
...
export function isMap(type: Type): boolean {
    return type.startsWith(
        "0x5afccfc2e6d32ec3a2066406035dc1494959a66d3c679f8069fe765640d4ceb1::map::Map<"
    )
}
...
export class Map<Key> {
    ...
    static fromFields<Key>(typeArgs: [Type, Type], fields: Record<string, any>): Map<Key> {
        initLoaderIfNeeded()
    ...
    static fromBcs<Key>(
        ...
        return Map.fromFields(typeArgs, bcs.de([Map.$typeName, ...typeArgs], data, encoding))
    }
    ...
    static async fetch<Key>(provider: JsonRpcProvider, id: ObjectId): Promise<Map<Key>> {
        ...
        if (res.data?.content?.dataType !== "moveObject" || !isMap(res.data.content.type)) {
